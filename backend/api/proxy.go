...
func HandleChatCompletion(rdb *redis.Client) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// Model override removed — respect client's requested model
		...
		// Store a clean copy of the body BEFORE provider-specific mutations for potential fallback
		cleanBodyBytes, _ := json.Marshal(body)
		...
		// Dynamic timeout: DeepSeek gets a moderate timeout to balance latency vs reliability
		...
		// If the upstream fails (timeout, connection refused) or returns a 50x error
		if err != nil || resp.StatusCode >= 500 {
			origStatus := 0
			if resp != nil {
				origStatus = resp.StatusCode
			}
			log.Printf("[%s] Upstream error (%s, status: %d): %v. Attempting Auto-Fallback...", clientID, provider, origStatus, err)

			// Determine fallback provider (Prioritize OpenAI, then Gemini)
			fallbackProvider := ""
			fallbackModel := ""
			var fallbackAPIKey string

			if provider != ProviderOpenAI {
				fallbackAPIKey, _ = GetProviderKey(tenantID, ProviderOpenAI)
				if fallbackAPIKey == "" {
					fallbackAPIKey, _ = rdb.HGet(context.Background(), "config:gateway", "openai_api_key").Result()
					if fallbackAPIKey == "" {
						fallbackAPIKey = os.Getenv("OPENAI_API_KEY")
					}
				}
				if fallbackAPIKey != "" {
			...
			if fallbackProvider == "" && provider != ProviderGemini {
				fallbackAPIKey, _ = GetProviderKey(tenantID, ProviderGemini)
				if fallbackAPIKey == "" {
					fallbackAPIKey, _ = rdb.HGet(context.Background(), "config:gateway", "gemini_api_key").Result()
					if fallbackAPIKey == "" {
			...
			if fallbackProvider != "" {
				log.Printf("[%s] Auto-Fallback triggered: %s -> %s (%s)", clientID, provider, fallbackProvider, fallbackModel)

				var fallbackBody map[string]interface{}
				// Use the clean body without provider-specific prior mutations
				json.Unmarshal(cleanBodyBytes, &fallbackBody)
				fallbackBody["model"] = fallbackModel

				fallbackTargetURL := GetProviderURL(fallbackProvider) + "/chat/completions"
				var fallbackReqBytes []byte

				if fallbackProvider == ProviderGemini {
					fallbackBody["model"] = "models/" + fallbackModel
					fallbackReqBytes, _ = json.Marshal(fallbackBody)
				} else {
					fallbackReqBytes, _ = json.Marshal(fallbackBody)
				}

				fallbackReq, fallbackReqErr := http.NewRequest("POST", fallbackTargetURL, bytes.NewReader(fallbackReqBytes))
				if fallbackReqErr == nil {
					fallbackReq.Header.Set("Content-Type", "application/json")
					fallbackReq.Header.Set("Authorization", "Bearer "+fallbackAPIKey)

					fallbackClient := &http.Client{Timeout: 300 * time.Second}
					fallbackResp, fallbackErr := fallbackClient.Do(fallbackReq)
					if fallbackErr == nil && fallbackResp.StatusCode < 500 {
						...
						model = fallbackModel
						reqBodyBytes = fallbackReqBytes
						log.Printf("[%s] Auto-Fallback to %s successful (Status: %d)", clientID, provider, resp.StatusCode)
					} else {
						log.Printf("[%s] Auto-Fallback to %s failed: %v", clientID, fallbackProvider, fallbackErr)
		...
		if err != nil {
			log.Printf("[%s] Upstream/Fallback error (%s): %v", clientID, provider, err)
		...
		if isStreaming {
			...
			if len(secretMap) > 0 {
				// When secrets exist, buffer the entire stream, rehydrate, then send
				fullBody, _ := io.ReadAll(resp.Body)
				fullStr := string(fullBody)

				// Concatenate all content deltas to get the full AI response text
				var fullContent strings.Builder
				type contentLoc struct {
					start, end int // byte offsets and original content within fullStr
					original   string
				}
				var locs []contentLoc

				// Find all "content":"..." values in the SSE stream
				contentKey := `"content":"`
				searchFrom := 0
				for {
					idx := strings.Index(fullStr[searchFrom:], contentKey)
					if idx < 0 {
						break
					}
					valStart := searchFrom + idx + len(contentKey)
					// Find the closing quote (handle escaped quotes)
					valEnd := valStart
					for valEnd < len(fullStr) {
						if fullStr[valEnd] == '\\' {
					...
					content := fullStr[valStart:valEnd]
					fullContent.WriteString(content)
					locs = append(locs, contentLoc{start: valStart, end: valEnd, original: content})
					searchFrom = valEnd + 1
				}

				// Rehydrate the full concatenated content
				assembled := fullContent.String()
				rehydrated := services.RehydrateSecrets(context.Background(), assembled, secretMap, rdb)

				if rehydrated != assembled {
					// Tokens were replaced — patch the SSE chunks
					// Strategy: put all rehydrated content into the first content chunk,
					// empty out the rest, preserving SSE structure
					result := fullStr
					// Work backwards to preserve byte offsets
					for i := len(locs) - 1; i >= 0; i-- {
						loc := locs[i]
						if i == 0 {
							// First chunk gets the full rehydrated content
							// Content is already JSON-encoded (extracted from raw SSE JSON),
							// and replacement values (UUIDs, emails, etc.) don't need escaping
							result = result[:loc.start] + rehydrated + result[loc.end:]
						} else {
							// Subsequent chunks become empty strings
							result = result[:loc.start] + result[loc.end:]
						}
					}
					c.Response().BodyWriter().Write([]byte(result))
				} else {
					// No tokens to replace — send original
					c.Response().BodyWriter().Write(fullBody)
				}
			} else {
				// No secrets to rehydrate — fast passthrough
				io.Copy(c.Response().BodyWriter(), resp.Body)
			}

			// Log stream
			latency := time.Since(startTime)
			go services.LogRequestUsage(tenantID, latency.Milliseconds(), resp.StatusCode >= 400, 0)
